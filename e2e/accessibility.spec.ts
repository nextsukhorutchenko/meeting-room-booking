...
test('@schedule keyboard entry and schedule semantics remain deterministic', async ({
  ...
  const jumpControls = [
    page.getByRole('combobox', {exact: true, name: 'День'}),
    page.getByRole('combobox', {exact: true, name: 'Час'}),
    page.getByRole('button', {exact: true, name: 'Перейти'}),
  ];
...
test('@booking visible targets, modal containment and restoration are stable', async ({
  ...
  const targetSizes = await page.locator(
    '.schedule-navigation-week-controls button:visible, ' +
    '.schedule-date-strip .schedule-date-button:visible, ' +
  ...
  expect(traversalCount).toBeGreaterThanOrEqual(1);
  await expect(slot).toBeFocused();
  await page.keyboard.press('Enter');
  const title = page.getByLabel('Назва');
  ...
  if (compact) {
    ...
    const end = dialog.getByLabel('Час завершення');
    const close = dialog.getByRole('button', {
      exact: true,
      name: 'Закрити',
    });
    const submit = dialog.getByRole('button', {name: 'Забронювати'});
    const closePanel = dialog.getByRole('button', {
...
test('@schedule independent error retry preserves focus and live-region priority', async ({
  ...
  const requestsBeforeRetry = scheduleRequests;
  await page.keyboard.press('Enter');
  ...
  expect(scheduleRequests - requestsBeforeRetry).toBe(1);
