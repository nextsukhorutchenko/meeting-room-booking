...
| Unit suite | PASS | `npm test`; 52 files and 553/553 non-database unit and browser-backed Chromium-unit tests |
| Coverage | PASS | `npm run test:coverage`; 81.84% statements, 79.10% branches, 78.69% functions, 82.76% lines |
...
`npm test` and `npm run test:coverage` remain database-free. Integration and E2E
retain the shared fail-fast preflight, require an explicit `_test`
`TEST_DATABASE_URL`, and do not fall back to `DATABASE_URL`.
...
| AC-035 | PASS | Unit 553/553, integration 106/106, E2E 141 passed with 5 intentional project-scope skips |
